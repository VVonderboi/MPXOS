...
/*
  Procedure..: printStringln
  Description..: Takes in a char* and calls sys_req to mitigate numerous int* declarations, adds new line
...
void printStringln(char* str){
	...
	int cntln =0;
	cntln= strlen(ln);
	sys_req(WRITE,DEFAULT_DEVICE, ln, &cntln);

}

/*
  Procedure..: printString
  Description..: Takes in a char* and calls sys_req to mitigate numerous int* declarations
  Params..: char *s -- String
*/
void printString(char* str){
	...
	cnt= strlen(str);
	//serial_println(str);
	sys_req(WRITE,DEFAULT_DEVICE, str, &cnt);
